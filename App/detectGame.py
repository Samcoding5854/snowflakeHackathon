...
#             st.session_state['image_url'] = image_url # Save image URL in session state
#         except:
...
#                 hint_key = f"Hint{i}"
#                 hint_key_with_space = f"Hints {i}"
        
#         # Check both possible keys
#                 if hint_key in hint:
#                     st.write(f"Hint {i}: {hint[hint_key]}")
#                 elif hint_key_with_space in hint:
#                     st.write(f"Hint {i}: {hint[hint_key_with_space]}")
...
def call_llm_api():
    input_data = {
        ...
        3. **Hints:** Offer a series of hints that can be revealed incrementally to guide the player in their investigation. These hints should be relevant to the story and suspects, providing subtle clues without giving away the solution outright.Atmost two hints
...
def DETECT_GAME():
    ...
    if st.button("Generate Story"):
        try:
            ...
            st.error(f"Error generating story or image:")

    # If response is available in session state, display the information
